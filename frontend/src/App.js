// Modified src/App.js to integrate better with custom model
...
export default function CrackDetectorApp() {
  ...
  const handleFileUpload = (e) => {
    ...
    if (file) {
      ...
      // Don't auto-upload, let user click Analyze button
    }
  };
  ...
  const uploadFile = async (file) => {
    try {
      ...
      if (response.data.success) {
        ...
        showNotification(`Analysis complete! ${response.data.detections.length} cracks detected.`, 'success');
  ...
  const processImageBlob = async (blob) => {
    try {
      ...
      const response = await axios.post('/api/camera', formData);
  ...
  const updateModel = async () => {
    ...
    if (file && file.name.endsWith('.pth')) {
      try {
        ...
        const response = await axios.post('/api/update_model', formData);
  ...
  const getSeverityColor = (severity) => {
    switch(severity) {
      case 'critical': return isDarkMode ? 'bg-red-900 text-red-200' : 'bg-red-100 text-red-800';
      case 'moderate': return isDarkMode ? 'bg-yellow-900 text-yellow-200' : 'bg-yellow-100 text-yellow-800';
      case 'minor': return isDarkMode ? 'bg-green-900 text-green-200' : 'bg-green-100 text-green-800';
      default: return isDarkMode ? 'bg-blue-900 text-blue-200' : 'bg-blue-100 text-blue-800';
    }
  };
    ...
    <div className={`min-h-screen ${isDarkMode ? 'bg-gray-900 text-white' : 'bg-gray-50 text-gray-800'}`}>
      ...
      <header className={`p-4 ${isDarkMode ? 'bg-gray-800' : 'bg-blue-600'} text-white shadow-md`}>
        <div className="container mx-auto flex justify-between items-center">
          ...
          <div className="flex space-x-4 items-center">
            ...
            <button 
              onClick={() => setIsModelUpdateOpen(true)}
              className="p-2 rounded-full hover:bg-blue-500 transition-colors"
            >
      ...
      <main className={`container mx-auto p-4 ${isFullscreen ? 'fixed inset-0 z-50 bg-black' : ''}`}>
        ...
        <div className={`flex rounded-t-lg overflow-hidden mb-4 ${isDarkMode ? 'bg-gray-800' : 'bg-white'} shadow`}>
          <button 
            className={`flex-1 py-3 px-4 flex items-center justify-center gap-2 transition-colors ${activeTab === 'upload' ? isDarkMode ? 'bg-blue-600 text-white' : 'bg-blue-500 text-white' : ''}`}
            onClick={() => {
              setActiveTab('upload');
          ...
          <button 
            className={`flex-1 py-3 px-4 flex items-center justify-center gap-2 transition-colors ${activeTab === 'camera' ? isDarkMode ? 'bg-blue-600 text-white' : 'bg-blue-500 text-white' : ''}`}
            onClick={() => {
              setActiveTab('camera');
              startCamera();
            }}
          ...
          <button 
            className={`flex-1 py-3 px-4 flex items-center justify-center gap-2 transition-colors ${activeTab === 'results' ? isDarkMode ? 'bg-blue-600 text-white' : 'bg-blue-500 text-white' : ''}`}
        ...
        <div className={`rounded-lg overflow-hidden shadow-lg ${isDarkMode ? 'bg-gray-800' : 'bg-white'} p-4`}>
          ...
          {activeTab === 'upload' && (
            <div 
              className={`border-2 border-dashed rounded-lg p-8 text-center ${isDarkMode ? 'border-gray-600' : 'border-gray-300'}`}
              onDrop={handleDrop}
                ...
                <div className="space-y-4">
                  ...
                  <div className="flex flex-col sm:flex-row gap-2 justify-center mt-4">
                    <button 
                      ...
                      className={`px-4 py-2 rounded-lg ${isDarkMode ? 'bg-blue-600 hover:bg-blue-700' : 'bg-blue-500 hover:bg-blue-600'} text-white flex items-center justify-center gap-2 transition-colors`}
                    >
                    ...
                    <button 
                      onClick={() => fileInputRef.current.click()}
                      className={`px-4 py-2 rounded-lg ${isDarkMode ? 'bg-purple-600 hover:bg-purple-700' : 'bg-purple-500 hover:bg-purple-600'} text-white flex items-center justify-center gap-2 transition-colors`}
                    >
                ...
                <div className="space-y-4">
                  ...
                  <div className="flex gap-2 justify-center">
                    ...
                    <button 
                      ...
                      className={`px-4 py-2 rounded-lg ${isDarkMode ? 'bg-green-600 hover:bg-green-700' : 'bg-green-500 hover:bg-green-600'} text-white transition-colors flex items-center gap-2`}
                    >
              ...
              <div className={`relative rounded-lg overflow-hidden bg-black ${isFullscreen ? 'fixed inset-0 z-50' : 'aspect-video'}`}>
                ...
                <div className="absolute bottom-4 inset-x-0 flex justify-center space-x-4">
                  <button 
                    ...
                    className="p-4 bg-red-500 rounded-full shadow-lg hover:bg-red-600 transition-colors flex items-center justify-center"
              ...
              {result ? (
                <>
                        ...
                        {detections.map(crack => (
                          ...
                          >
                            <div className="flex items-center gap-2">
                              <div className={`w-3 h-3 rounded-full ${getSeverityDotColor(crack.severity)}`}></div>
                              <span>Crack #{crack.id}</span>
                            </div>
                            <div className="flex items-center gap-4">
                              <span className="text-sm">
                                {crack.width.toFixed(2)} x {crack.height.toFixed(2)} cm
                              </span>
                  ...
                  <div className="flex justify-center gap-2">
                    <button 
                      className={`px-4 py-2 rounded-lg ${isDarkMode ? 'bg-blue-600 hover:bg-blue-700' : 'bg-blue-500 hover:bg-blue-600'} text-white transition-colors flex items-center gap-2`}
                      onClick={() => {
                    ...
                    <button 
                      className={`px-4 py-2 rounded-lg ${isDarkMode ? 'bg-purple-600 hover:bg-purple-700' : 'bg-purple-500 hover:bg-purple-600'} text-white transition-colors flex items-center gap-2`}
                      onClick={() => {
            ...
            <div className="space-y-4 mb-6">
              <div>
                ...
                <p className="text-xs text-gray-500 mt-1">
                  Lower values detect more cracks, higher values reduce false positives
                </p>
              </div>
              ...
              <div>
                ...
                <p className="text-xs text-gray-500 mt-1">
                  Filters out small artifacts that aren't actual cracks
                </p>
              </div>
            </div>
          ...
          <div className={`rounded-lg ${isDarkMode ? 'bg-gray-800' : 'bg-white'} p-6 max-w-md w-full shadow-xl`}>
            ...
            <div className={`border-2 border-dashed rounded-lg p-4 text-center ${isDarkMode ? 'border-gray-600' : 'border-gray-300'} mb-4`}>
              ...
              <button 
                ...
                className={`px-4 py-2 rounded-lg ${isDarkMode ? 'bg-blue-600 hover:bg-blue-700' : 'bg-blue-500 hover:bg-blue-600'} text-white flex items-center justify-center gap-2 transition-colors mx-auto`}
              >
            ...
            <div className="flex justify-end gap-2 mt-4">
              <button 
                onClick={() => setIsModelUpdateOpen(false)}
      ...
      {/* Notification */}
      {notification && (
        <div className={`fixed bottom-4 right-4 p-4 rounded-lg shadow-lg max-w-sm flex items-center gap-3 ${
          notification.type === 'success' 
            ? isDarkMode ? 'bg-green-800 text-green-100' : 'bg-green-100 text-green-800 border border-green-200'
            : isDarkMode ? 'bg-red-800 text-red-100' : 'bg-red-100 text-red-800 border border-red-200'
        }`}>
          {notification.type === 'success' ? (
